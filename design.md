...
| 写入阶段为每条 new memory 召回相似度达标的前 5 个 subject name，把逐 memory 前 2 与按 subject 最高分聚合的全局前 10 合并去重，再由一个批量 linking agent loop 覆盖全部 new memories。每条 memory 通常建立 `1～4` 个 link，并以 `direct` 或 `contextual` 记录依据。详细规则见 `design_detailed.md` 1.2.3 节 | 候选数量或相似度门槛造成漏召回、候选噪声、批次决策互相干扰，或两类 link 的判定不稳定 | 对召回参数与批次规模做消融，并分别评估漏链、错链、重复新 subject 与 `link_basis` 分类质量 |
| 批量 linking agent loop 最多调用 5 次主动 `association_search`。query 写成某条记忆可能改变或约束的另一主题名称（如 `Mike's dietary preferences`），而非复述新记忆；每轮保留原有 Subject + Memory 详细候选展示，搜到的可以是漏掉的 direct home 或 contextual 目标。详细规则见 `design_detailed.md` 1.2.3 节 | 常识上会跨主题约束的记忆持续漏链，或多轮主动检索引入过多 link 和推理成本 | 对关闭、单轮与多轮主动关联检索做消融实验，并评估关联召回率、错误 link 和调用成本 |
